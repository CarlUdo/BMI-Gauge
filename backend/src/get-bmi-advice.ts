...
export const getBmiAdvice = (bmi: number) => {
  if (bmi < 18.5) return bmiAdvice["underweight"];
  if (bmi >= 18.5 && bmi < 25) return bmiAdvice["healthyRange"];
  if (bmi >= 25 && bmi < 30) return bmiAdvice["overweight"];
  if (bmi >= 30 && bmi < 40) return bmiAdvice["obesity"];
  return bmiAdvice["severeObesity"];
};
